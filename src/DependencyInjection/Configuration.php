    ...
    public function getConfigTreeBuilder(): TreeBuilder
    {
        ...
        $rootNode
            ->children()
                ...
                ->arrayNode('data_source_cache')
                    ->addDefaultsIfNotSet()
                    ->children()
                        ->booleanNode('enabled')
                            ->defaultFalse()
                            ->info('Enable caching for data sources.')
                        ->end()
                        ->scalarNode('service')
                            ->defaultNull()
                ...
                ->arrayNode('mapping_cache')
                    ->addDefaultsIfNotSet()
                    ->children()
                        ->booleanNode('enabled')
                            ->defaultFalse()
                            ->info('Enable caching for mapping strategy conversions.')
                        ->end()
                        ->scalarNode('service')
                            ->defaultNull()
                            ->info('PSR-16 cache service ID (e.g., "cache.app").')
                ...
                ->arrayNode('mapping_strategy')
                    ->addDefaultsIfNotSet()
                    ->children()
                        ->booleanNode('enabled')
                            ->defaultFalse()
                            ->info('Enable mapping strategy feature for automatic field name conversion.')
                        ->end()
                    ->end()
                ->end()
